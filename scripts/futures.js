...
const { JSDOM } = jsdom;
const URL = "https://finance.yahoo.com/commodities";
...
async function createJSONObject(row) {
    const data = ["name", "lastPrice", "mktTime", "change", "pctChange", "volume", "uri"];

    const json = {};

    for (let i = 0; i < data.length; i++) {
        if (i == data.length - 1)
            json[data[i]] = `/futures/${row.children[0].textContent}`;
        else
